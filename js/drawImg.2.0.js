    ...
    img.onload = function () {
        ...
        if(that.sync=='async'){
                ...
                (function(n){
                    that.timer["l"+n]=setTimeout(function(){
                        ...
                        if(that.lineWidth>that.textPosition.width){
                            ctx.fillText(that.text.substring(that.lastSubStrIndex,n),that.textPosition.wx,(Number(that.initHeight)+Number(that.textPosition.hx)));//绘制截取部分
                            that.initHeight+=that.hHeight;//hHeight为字体的高度
                            that.lineWidth=0;
                            that.lastSubStrIndex=n;
                        }else{//绘制剩余部分
                            ctx.fillText(that.text.substring(that.lastSubStrIndex,n),that.textPosition.wx,(Number(that.initHeight)+Number(that.textPosition.hx)));
                        }
            ...
            for(let i=0;i<that.text.length;i++){
                ...
                if(that.lineWidth>that.textPosition.width){
                    ctx.fillText(that.text.substring(that.lastSubStrIndex,i),that.textPosition.wx,(Number(that.initHeight)+Number(that.textPosition.hx)));//绘制截取部分
                    that.initHeight+=that.hHeight;//20为字体的高度
                    that.lineWidth=0;
                    that.lastSubStrIndex=i;
                }
                if(i==that.text.length-1){//绘制剩余部分
                    ctx.fillText(that.text.substring(that.lastSubStrIndex,i),that.textPosition.wx,(Number(that.initHeight)+Number(that.textPosition.hx)));
                }
