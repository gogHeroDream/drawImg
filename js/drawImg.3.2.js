/*
  ...
  @param {Object}一个对象{
            ...
            *
 waterfall - v3.0 (2018-04-12T18:08:51+0800)
 *
...
function Watermark(obj){
    ...
    //文本颜色是否随机
    this.colorRandoming=obj.colorRandoming||false;
    // 文本字体
    ...
    this.list='';
    // 父元素DOM
    this.fatherBox=obj.fatherBox|| document.body;
    // 错误标识
    this.flag=false;
}
/*添加水印操作*/
Watermark.prototype.addWatermark = function (callback) {
    ...
    img.onload = function () {
        ...
        // 如果需要随机颜色，返回随机颜色
        (that.colorRandoming==true)&&(that.color=that.colorsRandom());
                ...
                (function(n){
                    that.timer["l"+n]=setTimeout(function(){
                        ...
                        if(that.lineWidth>that.textPosition.width){
                            ctx.fillText(that.text.substring(that.lastSubStrIndex,n+1),that.textPosition.wx,(Number(that.initHeight)+Number(that.textPosition.hx)),that.maxWidth);//绘制截取部分
                            that.initHeight+=that.hHeight;//hHeight为字体的高度
                            that.lineWidth=0;
                            that.lastSubStrIndex=n;
                        }else{//绘制

                            ctx.fillText(that.text.substring(that.lastSubStrIndex,n+1),that.textPosition.wx,(Number(that.initHeight)+Number(that.textPosition.hx)),that.maxWidth);
                        }
                        clearTimeout(that.timer["l"+n]);
                        //当绘制完再回调
                        if(n==(that.text.length-1)&&typeof callback === "function"){
...
// 取一个随机颜色
Watermark.prototype.colorRandom=function(){
...
// 取一个随机数
Watermark.prototype.numberRandom=function(){
    return Math.random();
}
// 返回需要的随机颜色的数组
Watermark.prototype.colorsRandom=function(){
    return [['0',this.colorRandom()],[this.numberRandom(),this.colorRandom()],['1.0',this.colorRandom()]];
}
/*canvas水印转化为图片*/
...
Watermark.prototype.init=function(){
    var that=this;
    that.fatherBox.appendChild(that.canvas);
    this.addWatermark(function(){
        // that.falg=false;
        // if(that.flag==true){
        //  that.falg=false;
        //  that.convertCanvasToImage();
        //  that.fatherBox.removeChild(that.canvas);
